...
mystery_string = "www.pythonchallenge.com/pc/def/map.html"

string.ascii_lowercase

for i in range(0,string.ascii_lowercase.__len__()):
    if i == string.ascii_lowercase.__len__()-2:
        mystery_string = mystery_string.replace(string.ascii_lowercase[i], string.ascii_uppercase[0])
    elif i == string.ascii_lowercase.__len__()-1:
        mystery_string = mystery_string.replace(string.ascii_lowercase[i], string.ascii_uppercase[1])
    else:
        mystery_string = mystery_string.replace(string.ascii_lowercase[i], string.ascii_uppercase[i+2])

answer1 = mystery_string

del mystery_string, i
...
for char in set(string):
    charcount[char] = string.count(char)
    
charcount = sorted(charcount.items(), key = lambda x:x[1])

replacelist = [char[0] for char in charcount if char[1] > 1]
...
del charcount, char, replacelist, string
...
while i < string.__len__()-8:
    if string[i].islower() & string[i+1:i+4].isupper() & string[i+4].islower() & string[i+5:i+8].isupper() & string[i+8].islower():
        answer3.append(string[i+1:i+8])
        i = i + 1
    else:
        i = i + 1
        
#answer3 = sorted(answer3)
...
for i in range(0,400):
    urlconnector = urllib.request.urlopen(url)
    text = str(urlconnector.read())
    PageContent.append(text)
    url = [s for s in url if not(s.isdigit())] + [s for s in text if s.isdigit()]
    url = "".join(url)
    
answer4 = PageContent[356]
